...
const LOCAL_DASHBOARD_XLSX_FILE = 'ms-global-request.xlsx';
...
async function loadDashboardFromPublishedHtml() {
  ...
  // 기존에는 실제 Excel 시트 1개가 사이드바 메뉴 1개였습니다.
  // MS 단일 시트 형식에서는 Model Name의 고유값 1개를 가상 시트 1개처럼 사용합니다.
  const virtualSheets = [];
  const loadErrors = [];

  sheetPayloads.forEach(function(payload, payloadIdx) {
    ...
    try {
      if (!hasHeadMarker(payload.matrix) && isMsFlatRequestMatrix(payload.matrix)) {
        ...
        virtualSheets.push({
          ...
          table: extractHeadMarkedTable(payload.matrix, 'sheet_' + (payloadIdx + 1), payload.styles || []),
          originalSheetName: payload.originalSheetName || ('sheet_' + (payloadIdx + 1))
        });
      }
    } catch (e) {
      const msg = '[' + (fallbackTitle || payload.sheetName || ('Sheet ' + (payloadIdx + 1))) + '] ' + (e && e.message ? e.message : e);
      loadErrors.push(msg);
      console.warn('[sheet-loader] source sheet skipped:', msg);
    }
  });

  const validSheets = [];
  virtualSheets.forEach(function(entry, idx) {
    const payload = entry.payload;
    const displayTitle = entry.displayTitle || ('Model ' + (idx + 1));
    const key = 'sheet_' + (idx + 1);
    try {
      const dashboardData = buildSheetDashboardData(entry.table, displayTitle);
      ...
      const msg = '[' + displayTitle + '] ' + (e && e.message ? e.message : e);
      loadErrors.push(msg);
      console.warn('[sheet-loader] model group skipped:', msg);
    }
  });
  ...
  // 정렬 후 sidebar key가 화면 순서와 일치하도록 key를 다시 지정합니다.
  validSheets.forEach(function(sheet, idx) { sheet.key = 'sheet_' + (idx + 1); });
...
async function loadSheetsFromPublishedXlsx(xlsxUrl) {
  ...
  (wb.SheetNames || []).forEach(function(sheetName, idx) {
    ...
    const isFlatMsRequest = isMsFlatRequestMatrix(cleaned);
    const displaySheetName = isFlatMsRequest
      ? 'MS Global Request'
      : getWorksheetB1DisplayTitle(ws, matrix, fallbackSheetName, idx);
    if (hasHeadMarker(cleaned) || isFlatMsRequest) {
      const weeklyUpdateText = hasHeadMarker(cleaned) ? getWeeklyUpdateB2FromWorksheet(ws, matrix) : '';
      const requestWeekText = hasHeadMarker(cleaned) ? getRequestWeekB4FromWorksheet(ws, matrix) : '';
      payloads.push({
        ...
        weeklyUpdates: weeklyUpdateText ? extractWeeklyUpdateItemsFromText(weeklyUpdateText) : []
      });
    } else {
      console.info('[sheet-loader] xlsx sheet skipped because required headers were not found:', sheetName);
    }
  });
...
function normalizeMsFlatHeader(value) {
  return String(value || '').toLowerCase().replace(/[\s_\-/.#]+/g, '');
}

function findMsFlatHeaderRow(matrix) {
  const required = ['region', 'projectname', 'taskstatusinptt', 'modelname', 'pg', 'liveurl'];
  const maxRows = Math.min((matrix || []).length, 20);
  for (let r = 0; r < maxRows; r++) {
    const normalized = (matrix[r] || []).map(normalizeMsFlatHeader);
    if (required.every(function(name) { return normalized.indexOf(name) >= 0; })) return r;
  }
  return -1;
}

function isMsFlatRequestMatrix(matrix) {
  return findMsFlatHeaderRow(matrix) >= 0;
}

function getMsFlatRequestColumnMap(matrix) {
  const headerRowIndex = findMsFlatHeaderRow(matrix);
  if (headerRowIndex < 0) {
    throw new Error('필수 헤더를 찾지 못했습니다: Region, Project Name, Task Status in PTT, Model Name, Pg#, Live URL');
  }
  const rawHeaders = (matrix[headerRowIndex] || []).map(function(v) { return String(v || '').trim(); });
  const normalizedHeaders = rawHeaders.map(normalizeMsFlatHeader);
  function col(name) { return normalizedHeaders.indexOf(normalizeMsFlatHeader(name)); }
  return {
    headerRowIndex: headerRowIndex,
    region: col('Region'),
    country: col('Project Name'),
...
function extractMsFlatRequestGroups(matrix, styles) {
  const cols = getMsFlatRequestColumnMap(matrix);
  const groups = [];
  const groupByTitle = Object.create(null);
  const tableHeaders = ['Region', 'Country', 'Status', 'Page#', 'URL'];

  for (let r = cols.headerRowIndex + 1; r < (matrix || []).length; r++) {
    const sourceRow = matrix[r] || [];
    const modelName = getMsFlatCell(sourceRow, cols.model);
    const region = getMsFlatCell(sourceRow, cols.region);
    const country = getMsFlatCell(sourceRow, cols.country);
    ...
    const row = {
      ...
      URL: url,
      __styles: {
        Region: normalizeFillColor(styles[r] && cols.region >= 0 ? styles[r][cols.region] : ''),
        Country: normalizeFillColor(styles[r] && cols.country >= 0 ? styles[r][cols.country] : ''),
        Status: normalizeFillColor(styles[r] && cols.status >= 0 ? styles[r][cols.status] : ''),
        'Page#': normalizeFillColor(styles[r] && cols.pages >= 0 ? styles[r][cols.pages] : ''),
        URL: normalizeFillColor(styles[r] && cols.url >= 0 ? styles[r][cols.url] : '')
      }
    };
...
// 기존 호출과의 호환을 위한 단일 테이블 변환입니다.
// 실제 MS 대시보드에서는 extractMsFlatRequestGroups()로 Model Name별 분리합니다.
function extractMsFlatRequestTable(matrix, styles) {
  const groups = extractMsFlatRequestGroups(matrix, styles);
  if (!groups.length) return { headers: ['Region', 'Country', 'Status', 'Page#', 'URL'], rows: [], headerRows: [['Region', 'Country', 'Status', 'Page#', 'URL']], headerStyleRows: [[]] };
  const rows = [];
  groups.forEach(function(group) { rows.push.apply(rows, group.table.rows); });
  return {
    headers: groups[0].table.headers.slice(),
    rows: rows,
    headerRows: groups[0].table.headerRows.map(function(row) { return row.slice(); }),
    headerStyleRows: [[]]
  };
}
...
function buildSheetDashboardData(table, sheetTitle) {
  ...
  const items = prepared.rows.map(function(row) {
    ...
    return {
      ...
      url: pickValue(row, ['URL', 'Url', 'url', 'Page URL', 'Live URL']) || '',
...
function pickCountryValue(row) {
  return pickValue(row, [
    ...
    'PDP Country', 'Locale', 'locale', 'Market', 'market', '법인', 'Subsidiary', 'Project Name'
  ]);
}
...
function switchModelTypeTab(division, button) {
  ...
  if (typeof window.switchMenu === 'function') {
    window.switchMenu(first);
  } else {
    document.querySelectorAll('#sheetNavList .nav-item').forEach(function(item) { item.classList.remove('active'); });
    first.classList.add('active');
    forceCurrentSheetTitle(window.__DEFAULT_DASHBOARD_KEY);
  }
}
window.switchModelTypeTab = switchModelTypeTab;
...
function renderSidebarNavFromSheets(keys) {
  ...
  updateModelTypeTabs(keys);

  var defaultKey = window.__DEFAULT_DASHBOARD_KEY || keys[0];
  document.querySelectorAll('#sheetNavList .nav-item').forEach(function(item) {
    item.classList.toggle('active', item.getAttribute('data-key') === defaultKey);
  });
}
...
function normalizeStatus(value) {
  ...
  if (/closed|complete|completed|done|완료|등록완료/.test(v)) return 'Done';
  if (/client\s*review|corp\.?\s*review|법인\s*리뷰|subsidiary\s*review/.test(v)) return 'Corp. Review';
  if (/in\s*progress|\bwip\b|working|작업중|진행중/.test(v)) return 'In Progress';
  if (/cancelled|canceled|cancel|취소/.test(v)) return 'Cancel';
  if (/pre[\s-]*check|pre[\s-]*review|request\s*clarification|사전\s*검토/.test(v)) return 'Pre-Review';
  return 'Pre-Review';
}
